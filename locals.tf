locals {
  ...
  postgres_uri  = var.rds_endpoint != "" ? "postgresql+psycopg2://${var.rds_username}:${var.rds_password}@${local.ext_pg_uri}" : (var.airflow_executor == "Sequential" ? "" : "postgresql+psycopg2://${var.rds_username}:${var.rds_password}@${aws_db_instance.airflow[0].address}:${aws_db_instance.airflow[0].port}/${aws_db_instance.airflow[0].db_name}")
